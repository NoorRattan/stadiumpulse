...
import type { PublicExperienceSection } from "./PublicExperiencePage";
import type { AmenityInfo, PublicExperienceResponse } from "@/types/api";
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {data.matches.map((match) => (
          ...
          >
            ...
            <p className="mt-2 text-sm font-semibold">{match.venueName}</p>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {data.venues.map((venue) => (
          <article
            ...
            key={venue.venueId}
          >
            <MapPinned aria-hidden="true" className="size-6 text-primary" />
            <h2 className="mt-5 font-display text-2xl font-bold">
              {venue.name}
            </h2>
            <p className="mt-2 text-sm text-muted-foreground">
              {venue.city}, {venue.country} · {venue.capacity.toLocaleString()}{" "}
              seats
            </p>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {data.venues.map((venue) => (
          <article
            ...
            key={venue.venueId}
          >
            <Accessibility aria-hidden="true" className="size-6 text-primary" />
...
function EventsSection({
  ...
  return (
    <section aria-label="Fan events" className="grid gap-4 lg:grid-cols-3">
      {data.fanEvents.map((event) => (
        ...
        >
          ...
          <h2 className="mt-5 font-display text-xl font-bold">{event.title}</h2>
  ...
  return (
    <section aria-label="Current safety notices" className="grid gap-4">
        ...
        >
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">
              {alert.severity} · {alert.zone} · {formatDate(alert.issuedAt)}
            </p>
            <h2 className="mt-2 font-display text-xl font-bold">
              {alert.title}
            </h2>
      ...
      <p className="rounded-xl border border-destructive/30 bg-destructive/8 p-4 text-sm">
        For an actual emergency, follow on-site staff instructions and contact
        local emergency services. StadiumPulse is not an emergency channel.
      </p>
    </section>
  );
}
...
export function PublicExperienceContent({
  data,
  section,
}: {
  data: PublicExperienceResponse;
  ...
  if (section === "matches") return <MatchesSection data={data} />;
  if (section === "venues") return <VenuesSection data={data} />;
  if (section === "accessibility") return <AccessibilitySection data={data} />;
  if (section === "amenities") return <AmenitiesSection data={data} />;
  if (section === "events") return <EventsSection data={data} />;
  if (section === "sustainability")
    return <SustainabilitySection data={data} />;
  if (section === "alerts") return <AlertsSection data={data} />;
  return <HelpSection data={data} />;
}
