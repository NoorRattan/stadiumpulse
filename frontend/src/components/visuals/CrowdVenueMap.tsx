...
function VenueDiagram({ zones }: { zones: CrowdZoneSummary[] }) {
  return (
    <div className="grid place-content-center rounded-2xl bg-muted p-4">
      ...
      >
        ...
        <path
          ...
          strokeWidth="2"
        />
        {venueSignalPositions.map(([cx, cy], index) => {
...
function BandLegend(): JSX.Element {
  return (
...
function ZoneSignalButton({
  onSelect,
  selected,
  zone,
}: {
  onSelect?: (zone: CrowdZoneSummary) => void;
  selected: boolean;
  zone: CrowdZoneSummary;
}) {
  const style = bandStyle[zone.band];
  const density = Math.round(zone.currentDensityPct);
  return (
    <button
      aria-label={`${zone.name}, ${density} percent density, ${style.label}`}
