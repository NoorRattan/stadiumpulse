...
function useOperationalDigest(refreshToken: number) {
...
function DigestHeader(): JSX.Element {
  return (
...
type DigestItemData = OperationalDigestResponse["items"][number];

function DigestItem({ item, rank }: { item: DigestItemData; rank: number }) {
  return (
...
function DigestResult({ digest }: { digest: OperationalDigestResponse }) {
  return (
      ...
      {digest.items.length === 0 ? (
        ...
        <ol className="grid gap-3 lg:grid-cols-3">
          {digest.items.map((item, index) => (
            <DigestItem item={item} key={item.zoneId} rank={index + 1} />
          ))}
        </ol>
      )}
    </div>
...
export function OperationalDigest({
  ...
  const { digest, error } = useOperationalDigest(refreshToken);
  return (
    <section
